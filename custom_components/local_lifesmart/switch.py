    ...
    async def _send_command(self, value: int):
        ...
        try:
            ...
            if "timed out" in str(ex).lower():
                # If it's a timeout error, assume the command succeeded
                # since the device is actually changing state as per user feedback
                _LOGGER.warning("Command timed out but may have succeeded: %s", str(ex))
                self._available = True  # Keep the device available
                self._state = bool(value)  # Assume the state changed successfully
                self.async_write_ha_state()
            else:
                # For other errors, log as error and mark as unavailable
